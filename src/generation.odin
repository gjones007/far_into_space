...
generation_call :: proc() {
	...
	switch raylib.GetRandomValue(0, 2000) {
	case 1000 ..= 1200:
	...
	// case 21 ..= 25:
	// 	{
	// 		grade := raylib.GetRandomValue(0, 4)
	// 		pos := ring_positioning(get_player_pos(), 30, 50)
	// 		size := rand.float32_range(0.70, 1.5)
	...
	case 91 ..= 301:
		{
			if len(game.space_objects) < MAX_SPACEOBJECT_COUNT {
				...
				grade := raylib.GetRandomValue(0, 2)
				switch grade {
				case 0:
...
gen_spaceobject_crate :: proc(pos: linalg.Vector2f32, type: SpaceObjectType, lifetime: f32) {
	...
	body_def.is_awake = true
	body_def.user_data = rawptr(o)
	body_def.angle = rand.float32_range(0, 1)
	body_def.linear_damping = 0.1
	body_def.angular_damping = 0.1
	o.body_id = b2.create_body(game.world_id, &body_def)
	shape_def := b2.DEFAULT_SHAPE_DEF
	shape_def.density = 0.01
	shape_def.friction = 0.01
	shape_def.restitution = 0.01
	shape_def.user_data = rawptr(o)
	...
	case SpaceObjectType.REPAIR:
		{
			...
			o.type = .REPAIR
			o.is_cargo = true
			box := b2.make_box(.17, .17)
			shape_id := b2.create_polygon_shape(o.body_id, &shape_def, &box)
		}
	case SpaceObjectType.WEAPON:
		{
			...
			type_id := raylib.GetRandomValue(0, i32(len(missle_weapon_defs)) - 1)
	...
	case SpaceObjectType.SPECIAL:
		{
			...
			type_id := raylib.GetRandomValue(0, i32(len(bullets_weapon_defs)) - 1)
...
gen_asteroid :: proc(pos: linalg.Vector2f32, min_size: f32, max_size: f32) {
	...
	body_def.type = .Dynamic
	body_def.position = pos
	body_def.linear_damping = 0.01
	body_def.angular_damping = 0.01
	body_def.user_data = rawptr(a)
	body_def.angle = rand.float32_range(0, 2 * math.PI)
	body_def.linear_velocity = b2.Vec2{rand.float32_range(-1, 1), rand.float32_range(-1, 1)}
	body_def.angular_velocity = rand.float32_range(0, 2 * math.PI)
	a.body_id = b2.create_body(game.world_id, &body_def)

	shape_def := b2.DEFAULT_SHAPE_DEF
	shape_def.density = 0.1
	shape_def.friction = 0.3
	shape_def.user_data = rawptr(a)

	circle: b2.Circle
	circle.radius = a.size
	shape_id := b2.create_circle_shape(a.body_id, &shape_def, &circle)
...
ring_positioning :: proc(
	...
	for true {
		center =
			pos +
			linalg.Vector2f32 {
					rand.float32_range(-outer_ring, outer_ring),
					rand.float32_range(-outer_ring, outer_ring),
				}
...
gen_enemy :: proc(
	...
	body_def := b2.DEFAULT_BODY_DEF
	body_def.user_data = rawptr(e)
	body_def.type = .Dynamic
	body_def.position = pos
	body_def.linear_damping = 2.0
	body_def.angular_damping = 1.0
	e.body_id = b2.create_body(game.world_id, &body_def)

	shape_def := b2.DEFAULT_SHAPE_DEF
	shape_def.density = 0.01
	shape_def.friction = 0.03
	shape_def.user_data = rawptr(e)
