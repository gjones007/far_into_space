...
MISSIONS_DEFS := []MissionDef {
    MissionDef {
        ...
        tasks = []MissionTaskDef {
            ...
            MissionTaskDef{update_handler = mission_pickup_cargo, description = "Pickup a cargo box."},
            MissionTaskDef{update_handler = mission_use_cargo, description = "Use the item in the cargo box."},
            MissionTaskDef{update_handler = mission_return_to_base, description = "Return to base."},
        },
    },
    // MissionDef {
    // 	name = "Mission 1",
...
world_display_waypoint :: proc() {
    using raylib
    DrawCircleV(game.waypoint * RE_SCALE, 1 * RE_SCALE, Fade(ORANGE, rand.float32_range(0.25, 0.45)))
}
...
screen_navigation_waypoint_display :: proc() {
    ...
    if game.showing_waypoint_nav {
        ...
        waypoint_bearing := math.atan2_f32(game.waypoint.x - playerpos.x, game.waypoint.y - playerpos.y)

        center := raylib.Vector2{f32(GetScreenWidth() / 2), f32(GetScreenHeight() / 2)}
        offset := linalg.Vector2f32 {
            f32(textures[TEXTURES_ID.ARROW_UP_WHITE].width / 2),
            f32(textures[TEXTURES_ID.ARROW_UP_WHITE].height / 2) + 100,
        }
        ...
        if game.showing_display_panel || game.show_display_panel_forced_time > game.current_time {
            ...
            DrawTextEx(default_font, "[NAVIGATION OFFLINE]", NAV_LOCATION_POS, NAV_LOCATION_SIZE, 1, RED)
        }
    }
}
...
draw_mission_popup_panel :: proc() {
    using raylib
    DrawRectangle(0, raylib.GetScreenHeight() - 200, raylib.GetScreenWidth(), 200, Fade(WHITE, 0.5))
    DrawTextEx(default_font, MISSIONS_DEFS[game.mission].name, Vector2{10.0, f32(raylib.GetScreenHeight() - 150)}, 24, 1, WHITE)
    DrawTextEx(
        default_font,
    ...
    DrawTextEx(default_font, last_msg, Vector2{10.0, f32(raylib.GetScreenHeight() - 90)}, 30, 1, RED)
